...
public class Perfoma_timeCapacity_perfo_timeCapacity_Section extends
		AbstractSection {
	...
	public boolean select(Object eObject) {
		...
		if (eObjectToTest instanceof PerformanceCriteria
				|| eObjectToTest instanceof UnityElement)
			return true;
	...
	public void createControls(Composite parent,
		...
		perfo_timeCapacity_AttributeGroup = getWidgetFactory().createGroup(
				_rootParentComposite, "Time Capacity Attributes");
		perfo_timeCapacity_AttributeGroup.setLayout(new GridLayout(1, false));
		GridData gdperfo_timeCapacity_AttributeGroup = new GridData(
				GridData.FILL_HORIZONTAL);
		gdperfo_timeCapacity_AttributeGroup.horizontalSpan = ((GridLayout) _rootParentComposite
				.getLayout()).numColumns;
		perfo_timeCapacity_AttributeGroup
				.setLayoutData(gdperfo_timeCapacity_AttributeGroup);

		ValueField = new TextValueGroup(perfo_timeCapacity_AttributeGroup,
				"Time Limit :", getWidgetFactory(), true);

		MeasurementUnitField = new MeasurementUnitField_semanticKindGroup(
				perfo_timeCapacity_AttributeGroup, getWidgetFactory());
	...
	public void loadData(CapellaElement capellaElement_p) {
		...
		ValueField.loadData(capellaElement_p,
				PerfoPackage.eINSTANCE.getPerformanceCriteria_Value());

		MeasurementUnitField.loadData(capellaElement_p,
				PerfoPackage.eINSTANCE.getUnityElement_MeasurementUnit());
